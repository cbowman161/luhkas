...
def _recipe_prompt(*, user_input: str, payload: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "  • Paired bluetooth devices — DON'T `bluetoothctl paired-devices` (deprecated subcommand).\n"
        "    DO `bluetoothctl devices Paired` (current syntax) or python_script wrapping it.\n"
        "  • Bluetooth on/off — `bluetoothctl show` prints `Powered: yes/no`; or `rfkill list bluetooth`.\n"
        "  • Wifi current SSID — `iwgetid -r` returns the connected SSID; `nmcli -t -f active,ssid dev wifi` for verbose.\n"
        "  • Audio devices — `pactl list short sinks` and `pactl list short sources`.\n"
        "  • USB devices — `lsusb` (no flags needed).\n"
        "  • PCI devices — `lspci` (no flags) or `lspci -nn` for IDs.\n"
        "  • Kernel modules — `lsmod` (no flags); first line is the header.\n"
        "  • CGroups — `cat /proc/self/cgroup` for the current process, `systemd-cgls --no-pager` for the tree.\n"
        "  • Battery — `cat /sys/class/power_supply/BAT*/capacity` or `acpi -b` if present.\n"
        "  • Sensors / temperatures — `sensors` if lm-sensors installed, else read /sys/class/thermal/thermal_zone*/temp.\n\n"
